...
private final class ImageViewController {
    ...
    private func _display(_ image: Image, _ transition: ImageLoadingOptions.Transition?, _ alwaysTransition: Bool, _ fromMemCache: Bool, _ newContentMode: UIView.ContentMode?) {
        guard let pokemonImageView = imageView else { return }
        ...
        if !fromMemCache || alwaysTransition, let transition = transition {
            ...
            case let .custom(closure):
                ...
                // animations.
                closure(pokemonImageView, image)
            }
        } else {
            pokemonImageView.display(image: image)
        }
        if let newContentMode = newContentMode {
            pokemonImageView.contentMode = newContentMode
        }
    }
    ...
    private func _runFadeInTransition(image: Image, params: ImageLoadingOptions.Transition.Parameters, contentMode: UIView.ContentMode?) {
        ...
        if let contentMode = contentMode, pokemonImageView.contentMode != contentMode, let pokemonImageView = pokemonImageView as? UIImageView, pokemonImageView.image != nil {
            _runCrossDissolveWithContentMode(pokemonImageView: pokemonImageView, image: image, params: params)
        } else {
    ...
    private func _runCrossDissolveWithContentMode(pokemonImageView: UIImageView, image: Image, params: ImageLoadingOptions.Transition.Parameters) {
        ...
        transitionView.image = pokemonImageView.image
        transitionView.contentMode = pokemonImageView.contentMode
        pokemonImageView.addSubview(transitionView)
        transitionView.frame = pokemonImageView.bounds

        // "Manual" cross-fade.
        transitionView.alpha = 1
        pokemonImageView.alpha = 0
        pokemonImageView.image = image // Display new image in current view
